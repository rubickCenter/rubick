...
const createPluginManager = (): any => {
  ...
  const { options, searchFocus, setOptionsRef, clipboardFile, clearClipboardFile, readClipboardContent } = optionsManager({
    searchValue,
    appList,
    openPlugin,
  ...
  const getPluginInfo = async ({ pluginName, pluginPath }) => {
    ...
    return {
      ...
      indexPath: commonConst.dev() ? 'http://localhost:8081/#/' : `file://${path.join(pluginPath, '../', pluginInfo.main)}`
    };
  };
